...
class PackageDependency < ApplicationRecord
  ...
  belongs_to :dependency,
             ...
             # Optional because we might not know about this package
             optional:    true
  ...
  scope :preloaded_for_health_status, lambda {
    ...
    includes(dependency_project: %i[package github_repo])
  }

  #
  # Retrieves the sets of dependencies for the given project wrapped in
  # a PackageDependency::Collection with associations preloaded to
  # easily assess project health status based on the returned data
  #
  def self.for_project(project)
    return Collection.new(development: [], runtime: []) if project.package_name.blank?

    base_scope = where(package_name: project.package_name).strict_loading.preloaded_for_health_status
