...
RSpec.describe PackageCodeStatsJob do
  fixtures :all
  ...
  before do
    allow(PackageCodeStatsService).to receive(:statistics)
      .with(name: package.name, version: package.current_version)
      .and_return fake_statistics
  end

  describe ".perform" do
    subject(:perform) { job.perform package.name }

    it do
      expect { perform }
        .to change { package.code_statistics.pluck(:language, :code) }
        .from([])
        .to([["c", 300], ["ruby", 100]])
    end
    ...
    context "when we have some dropped language in the database" do
      ...
      it do
        expect { perform }
          .to change { package.code_statistics.pluck(:language) }
