...
RSpec.describe Package::Trend::Navigation do
  fixtures :all

  let(:package) { Factories.package "example" }

  before do
    Factories.package_trend package.name, date: 56.weeks.ago, position: 1
    Factories.package_trend package.name, date: 8.weeks.ago, position: 1
    Factories.package_trend package.name, date: 4.weeks.ago, position: 1
    Factories.package_trend package.name, date: 3.weeks.ago, position: 1
    Factories.package_trend package.name, date: 1.week.ago, position: 1
    Factories.package_trend package.name, date: Time.current, position: 1
  end
  ...
  describe ".latest_date" do
    it "is the latest available date" do
      allow(Package::Trend).to receive(:maximum).and_return(Date.new(2019, 2, 24))
