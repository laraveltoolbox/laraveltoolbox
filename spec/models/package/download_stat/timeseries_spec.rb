...
RSpec.describe Package::DownloadStat::Timeseries do
  fixtures :all

  let(:package) { Factories.package "example" }

  before do
    package.download_stats.create! date: 12.weeks.ago, total_downloads: 1000
    package.download_stats.create! date: 8.weeks.ago, total_downloads: 2000
    package.download_stats.create! date: 4.weeks.ago, total_downloads: 3000
    package.download_stats.create! date: 1.week.ago, total_downloads: 3750
    package.download_stats.create! date: Time.current, total_downloads: 4000
  end
  ...
  describe "#stats" do
    let(:timeseries) do
      described_class.new(package.name, :total_downloads, :absolute_change_month)
    end
    ...
    it "makes a single database query" do
      expect { timeseries.stats }.to make_database_queries(matching: "SELECT \"package_download_stats\"", count: 1)
    end
