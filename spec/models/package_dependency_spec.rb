...
RSpec.describe PackageDependency do
  fixtures :all
  ...
  describe "associations" do
    it {
      expect(model).to belong_to(:package)
        ...
        .inverse_of(:package_dependencies)
    }
  ...
  describe ".for_project" do
    ...
    it "is an empty PackageDependency::Collection when project has no package name" do
      allow(project).to receive(:package_name)
      expect(described_class.for_project(project))
        .to be_a(described_class::Collection)
        .and have_attributes(development: [], runtime: [])
    end

    describe "when the project has a package_name" do
      let(:base_scope) do
        described_class.where(package_name: project.package_name).preloaded_for_health_status
      end

      before do
        allow(project).to receive(:package_name).and_return(SecureRandom.hex(8))
      end
