...
RSpec.describe Project::ForkDetector do
  ...
  describe "for a project that references a significantly more popular gem via it's github repo" do
    let(:package) { instance_double(Package, name: "foo", downloads: 500_000) }

    let(:project) do
      instance_double Project,
                      github_repo_sibling_gem_with_most_downloads: package,
                      package_downloads:                           30,
    ...
    it "references the expected package as forked_from" do
  ...
  describe "for a project that has the same description as a much more popular gem" do
    let(:project) do
      Package.create! name: "demo", current_version: "1.2", description: "Hello World", downloads: 50_000
      instance_double Project,
    ...
    it "has package_sibling in fork_critera" do
      expect(detector.fork_criteria).to eq %w[package_sibling]
    end
    ...
    it "isn't a fork when the inspected project has more than 50k downloads" do
      allow(project).to receive(:package_downloads).and_return(50_001)
