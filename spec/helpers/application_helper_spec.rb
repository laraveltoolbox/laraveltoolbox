...
RSpec.describe ApplicationHelper do
  ...
  describe "#metrics_row" do
    let(:project) { instance_double(Project, package_downloads: 22_123_122) }
    let(:metrics_row) { helper.metrics_row(project, :package_downloads) }
    ...
    it "renders floats floored and pretty-printed as a percentage" do
      allow(project).to receive(:package_downloads).and_return(1297.7)
      expect(metrics_row).to include("1,297%")
    end

    it "renders dates regularly" do
      allow(project).to receive(:package_downloads).and_return(Date.new(2014, 7, 4))
      expect(metrics_row).to include("2014-07-04")
    end

    it "renders times as dates" do
      allow(project).to receive(:package_downloads).and_return(Time.utc(2014, 7, 4, 13, 13, 0))
      expect(metrics_row).to include("2014-07-04")
    end

    it "renders strings as-is" do
      allow(project).to receive(:package_downloads).and_return("Hello")
      expect(metrics_row).to include "Hello"
    end

    it "renders arrays as sentences" do
      allow(project).to receive(:package_downloads).and_return(%w[Hello World])
      expect(metrics_row).to include "Hello and World"
    end

    it "renders only the container when the value is blank" do
      allow(project).to receive(:package_downloads)
      expect(metrics_row).to eq "<div class=\"metric\" data-metric-name=\"package_downloads\"></div>"
    end
  end
